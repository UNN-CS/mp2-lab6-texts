...
void TTextLink::InitMemSystem(int size) { // инициализаци€ пам€ти
	char Line[100];
	MemHeader.pFirst = (PTTextLink) new char[sizeof(TTextLink)*size];
	MemHeader.pFree = MemHeader.pFirst;
	MemHeader.pLast = MemHeader.pFirst + (size - 1);
	PTTextLink pLink = MemHeader.pFirst;
	for (int i = 0; i < size - 1; i++, pLink++) { // размерка пам€ти
		pLink->pNext = pLink + 1;
	}
	pLink->pNext = NULL;
}
...
void TTextLink:: MemCleaner( TText &txt) {// сборка мусора
	...
	for (; pLink <= MemHeader.pLast; pLink++) {
		if (strstr(pLink->Str, "&&&") != nullptr) //cтрока текста или свободное звено
			strcpy(pLink->Str, pLink->Str + 3); //сн€тие маркировки
		else
			delete pLink;
	}
}
